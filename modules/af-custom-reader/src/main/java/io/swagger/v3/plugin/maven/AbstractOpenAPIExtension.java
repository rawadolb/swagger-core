...
public abstract class AbstractOpenAPIExtension implements OpenAPIExtension {
    ...
    public ResolvedParameter extractParameters(List<Annotation> annotations, Type type, Set<Type> typesToSkip,
                                               Components components, boolean includeRequestBody, JsonView jsonViewAnnotation, Iterator<OpenAPIExtension> chain) {
        if (chain.hasNext()) {
            return chain.next().extractParameters(annotations, type, typesToSkip, components, includeRequestBody, jsonViewAnnotation, chain);
        } else {
